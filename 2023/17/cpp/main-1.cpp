...
void solve([[maybe_unused]] int _t) {
  ...
  while (!pq.empty()) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (dr >= 0 and dr < n and dc >= 0 and dc < m and new_count <= 3 and !have.count({dr, dc, i, new_count}) and (direction == -1 or index_to_direction[i] != opposite_direction[index_to_direction[direction]])) {
        int d = grid[dr][dc] - '0';
